...
bool Geometry::loadOBJ(const FilePath& filepath, const FilePath& mtlBasePath, bool loadTextures) {
    ...
    log << "Load OBJ " << filepath << std::endl;
    ...
    log << "Number of meshes: " << shapes.size() << std::endl;
    log << "Number of vertices: " << nbVertex << std::endl;
    log << "Number of triangles: " << (nbIndex) / 3 << std::endl;
